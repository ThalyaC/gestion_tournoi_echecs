...
def display_on_screen_round():
    ...
    try:
        ...
        try:
            ...
            display_round_3(content_round, list_players_round, list_chess_players)
        except IndexError:
            print("Cette ronde n'a pas été encore créée.")
    except ValueError:
    ...
    for table in list_pairing1:
        ...
        if player_black3 == "exempt":
            ...
            espace_number_black = 25 - (
                (len(player_black1["Nom"]) + len(player_black1["Prenom"])) + len(table["score"])
                )
            espace = " "
            espaces_b = espace_number_black * espace
